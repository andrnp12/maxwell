function getChartColorsArray(r) {
  ...
  // 1. Cek apakah elemen ada di HTML
  if (element.length === 0) {
    console.warn("Elemen " + r + " tidak ditemukan di halaman.");
    return []; // Kembalikan array kosong agar tidak error
  }

  var colorsAttr = element.attr("data-colors");

  // 2. Cek apakah atribut data-colors ada
  if (!colorsAttr) {
    console.warn("Atribut data-colors tidak ditemukan pada " + r);
    return [];
  }

  try {
    // 3. Gunakan try-catch agar jika JSON salah format, script tidak berhenti total
    return JSON.parse(colorsAttr).map(function (color) {
      color = color.replace(" ", "");
      if (-1 == color.indexOf("--")) return color;
      var computedColor = getComputedStyle(
        document.documentElement,
      ).getPropertyValue(color);
      return computedColor || void 0;
    });
  } catch (e) {
    console.error("Gagal parse JSON warna untuk " + r + ": ", e);
    return [];
  }
}
